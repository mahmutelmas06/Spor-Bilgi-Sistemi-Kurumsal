...
// @version      1.0
// @description  Sporcu Ara için TC eşleşmesi ve boş tablo reddi (Doğrulama)
        ...
        iframe.onload = function () {
            ...
            var tid = setInterval(function () {
                ...
                try {
                    ...
                    if (guncelHash !== oncekiHash) {
                        oncekiHash = guncelHash;
                        stabilSayac = 0;
                        return;
                    }
                ...
                } catch (e) { console.error('[GSB v26]', e); }
            }, 500);
        };
    }
        ...
        function init() {
            ...
            tablo.querySelectorAll('tbody tr').forEach(function (tr) {
                ...
                if (tr.children.length >= 15) {
                    tr.insertBefore(td, tr.children[15]);
                } else {
                    tr.appendChild(td);
                }
        ...
        function isle(is, cid) {
            ...
            detaySayfasiniYukle(
                ...
                function () {
                    ...
                    var isKopya = is;
                    var cidKopya = cid;
                    yenidenDeneTD(isKopya.td, function () {
                        isKopya.td.innerHTML = '<span style="color:#007bff;font-size:0.9em;"><i class="fa fa-spinner fa-spin"></i> Taranıyor...</span>';
                        if (isKopya.sporcuTd) {
                            var o = isKopya.sporcuTd.getAttribute('data-orijinal');
                            isKopya.sporcuTd.innerHTML = '<div style="text-align:center;"><strong>' + o + '</strong><br><span style="color:#888;font-size:0.9em;"><i class="fa fa-spinner fa-spin"></i> Sırada...</span></div>';
                        }
                        aktifIsSayisi++;
                        isle(isKopya, cidKopya);
                    });

                    if (isKopya.sporcuTd) {
                        var orj = isKopya.sporcuTd.getAttribute('data-orijinal');
                        isKopya.sporcuTd.innerHTML = '<div style="text-align:center;"><strong>' + orj + '</strong><br><small style="color:#f44336;font-weight:bold;">Zaman Aşımı / Hata</small></div>';
                    }
        ...
        function grupVerisiniYaz(idoc, is) {
            ...
            function tara(hedef) {
                ...
                hedef.querySelectorAll('thead th').forEach(function (th, i) {
                    ...
                    if (attr === 'EDevletBasvurusuMu' || metin.includes('DEVLET')) {
                        eDevInd = i;
                    }
                });
                ...
                rows.forEach(function (row) {
                    ...
                    if ((gorevMetni.includes('ANTRENÖR') ||
                        gorevMetni.includes('ÖĞRETMEN') ||
                        gorevMetni.includes('LİDER')) &&
                        !gorevMetni.includes('SPORCU')) {

                        var adSoyad = "";
                        if (row.cells.length > 4) {
                            adSoyad = temizle(row.cells[3].textContent + ' ' + row.cells[4].textContent);
                        } else {
                            adSoyad = temizle(row.cells[2].textContent);
                        }
            ...
            if (bekTablo) {
                ...
                if (bRows.length && !bRows[0].classList.contains('no-records-found'))
                    bekleyen = bRows.length;
            }

            is.td.innerHTML = gorevliler.length
                ? Array.from(new Set(gorevliler)).join(
            ...
            if (is.sporcuTd) {
                ...
                is.sporcuTd.innerHTML =
                    ...
                    (bekleyen > 0
                        ? '<br><small style="color:#ff9800;font-weight:bold;">Bekleyen: ' + bekleyen + '</small>'
                        : '') +
                    '</div>';
            }
        }
    ...
    function sporcuAraModuBaslat() {
        ...
        function init() {
            ...
            tablo.querySelectorAll('tbody tr').forEach(function (tr) {
                ...
                var sonTr = tr.lastElementChild;
                var tdA = tdEkle(tr, sonTr, 'sp-antrenor-td', 'font-size:0.88em;');
                var tdE = tdEkle(tr, sonTr, 'sp-edev-td', 'text-align:center;');
                var tdY = tdEkle(tr, sonTr, 'sp-yas-td', 'text-align:center;');

                var btn = tr.querySelector('input[onclick*="sporcuKartGrupDetayGit"]');
                if (!btn) {
                    tdA.innerHTML = '<span style="color:#aaa;">-</span>';
                    tdE.innerHTML = '-';
                    tdY.innerHTML = '-';
                    return;
                }
                ...
                // V26 FIX: TC numarası artık kusursuz eşleşme için toplanıyor
                ...
                isKuyrugu.push({
                    tdA: tdA, tdE: tdE, tdY: tdY,
                    url: url,
                    atletTc: atletTc,
                    atletAd: atletAd, atletSoyad: atletSoyad
                });
            });
        ...
        function kuyruguIsle(cid) {
            ...
            while (aktifIsSayisi < MAX_AKTIF_IS && isKuyrugu.length > 0) {
                ...
                if (grupCache[is.url]) {
                    tdleriBesle(is, grupCache[is.url]);
                    continue;
                }
                if (grupBekleyen[is.url]) {
                    grupBekleyen[is.url].push(is);
                    continue;
                }
        ...
        function isle(url, cid) {
            ...
            detaySayfasiniYukle(
                ...
                function (idoc) {
                    ...
                    // V26 FIX: Doğrulama - Eğer detay sayfasından hiç e-devlet verisi çıkmadıysa,
                    // sayfa eksik yüklenmiş demektir. Bu durumda boş tabloyu önbelleğe alıp
                    // geri kalan çocukları yakmak yerine, hata (tekrar dene) verdirtiyoruz!
        ...
        function veriyiCikar(idoc) {
            ...
            if (!tablo) return { gorevliler: gorevliler, edevletMap: edevletMap, yasMap: yasMap };
            ...
            tablo.querySelectorAll('tbody tr').forEach(function (row) {
                ...
                // V26 FIX: TC numarası detay sayfasından da çekiliyor
                ...
                if ((gorev.includes('ANTRENÖR') ||
                    gorev.includes('ÖĞRETMEN') ||
                    gorev.includes('LİDER')) &&
                ...
                if (gorev.includes('SPORCU')) {
                    if (row.cells.length > eDevInd) {
                        ...
                        // TC numarasını da alternatif bir yedek anahtar olarak kaydediyoruz
                        if (tc && tc.includes('*')) edevletMap[tc] = val;
                    }
            ...
            for (var g = 0; g < gorevliler.length; g++) {
                ...
                if (!gorulmus[k]) { gorulmus[k] = true; temiz.push(gorevliler[g]); }
            }

            return { gorevliler: temiz, edevletMap: edevletMap, yasMap: yasMap };
        }
        ...
        function tdleriBesle(is, veri) {
            ...
            // V26 FIX: Hem TC hem İsim üzerinden arama yapılıyor
            var tcAnahtar = temizle(is.atletTc);
            var adAnahtar = temizle(is.atletAd + ' ' + is.atletSoyad);

            // Önce garantili olan TC'ye bakıyoruz, bulamazsa isme bakıyoruz
